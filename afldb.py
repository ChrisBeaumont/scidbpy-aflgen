# Automatically generated by scidbpy-afljson
...
operators = [
  ...
  {
    "doc": "approxdc( srcArray [, attr] )\n\nProduces a result array containing approximate counts of the number of distinct values of an attribute.\n\nParameters\n----------\n\n    - srcArray: a source array with srcAttrs and srcDims.\n    - 0 or 1 attributes from srcAttrs. If no attribute is provided, the\n      first attribute is used.\n\nExamples\n--------\n\n    - Given array A <quantity: uint64, sales:double> [year, item] =\n       year, item, quantity, sales\n       2011, 2, 7, 31.64\n       2011, 3, 6, 19.98\n       2012, 1, 5, 41.65\n       2012, 2, 9, 40.68\n       2012, 3, 8, 26.64\n    - approxdc(A, sales) <sales_adc: uint64> [i] =\n       i, sales_adc\n       0, 5\n\nNotes\n-----\n\n    - When used inside aggregate(), groupby approxdc is possible; but\n      when used standalone, no dimension may be provided.\n    - The naming xxx_adc is inconsistent with the naming in\n      aggregate(...approxdc()). The latter uses xxx_approxdc. Should\n      change.", 
    "name": "ApproxDC", 
    "signature": [
      "array", 
      "args"
  ...
  {
    "doc": "avg( srcArray, [attr {, groupbyDim}*] )\n\nComputes the average value of an attribute. If no attribute is given, the first attribute is used. If a list of groupbyDims are given, compute one average value per distinct group.\n\nParameters\n----------\n\n    - srcArray: a source array with srcAttrs and srcDims.\n\nExamples\n--------\n\n    - Given array A <quantity: uint64, sales:double> [year, item] =\n       year, item, quantity, sales\n       2011, 2, 7, 31.64\n       2011, 3, 6, 19.98\n       2012, 1, 5, 41.65\n       2012, 2, 9, 40.68\n       2012, 3, 8, 26.64\n    - avg(A, sales, year) <sales_avg:double> [year] =\n       year, sales_avg\n       2011, 25.81\n       2012, 36.3233\n\nNotes\n-----\n\n    - same as aggregate(srcArray, avg(attr), groupbyDims)", 
    "name": "avg", 
    "signature": [
      "array", 
      "args"
  ...
  {
    "doc": "max( srcArray [, attr {, groupbyDim}*] )\n\nProduces a result array containing the max value among the values of an attribute. If a list of groupbyDims is provided, one max value is calculated for every distinct group.\n\nParameters\n----------\n\n    - srcArray: a source array with srcAttrs and srcDims.\n    - 0 or 1 attributes from srcAttrs. If no attribute is provided, the\n      first attribute is used.\n    - an optional list of groupbyDims.\n\nNotes\n-----\n\n    - same as aggregate(srcArray, max(attr), groupbyDims).", 
    "name": "max", 
    "signature": [
      "array", 
      "args"
  ...
  {
    "doc": "min( srcArray [, attr {, groupbyDim}*] )\n\nProduces a result array containing the min value among the values of an attribute. If a list of groupbyDims is provided, one min value is calculated for every distinct group.\n\nParameters\n----------\n\n    - srcArray: a source array with srcAttrs and srcDims.\n    - 0 or 1 attributes from srcAttrs. If no attribute is provided, the\n      first attribute is used.\n    - an optional list of groupbyDims.\n\nNotes\n-----\n\n    - same as aggregate(srcArray, min(attr), groupbyDims).", 
    "name": "min", 
    "signature": [
      "array", 
      "args"
  ...
  {
    "doc": "redimension_store( srcArray, outputArray {, AGGREGATE_CALL}* )\n            AGGREGATE_CALL := AGGREGATE_FUNC(inputAttr) [as resultName]\n            AGGREGATE_FUNC := approxdc | avg | count | max | min | sum | stdev\n           | var | some_use_defined_aggregate_function\n\nProduces a stored array using some or all of the variables of a source array, potentially changing some or all of those variables from dimensions to attributes or vice versa, and optionally calculating aggregates to be included in the new array.\n\nParameters\n----------\n\n    - srcArray: a source array with srcAttrs and srcDims.\n    - outputArray: a pre-existing output array, with outputAttrs and\n      outputDims. All the dimensions in outputDims must exist either in\n      srcAttrs or in srcDims, with one exception. One new dimension\n      called the synthetic dimension is allowed. All the attributes in\n      outputAttrs, which is not the result of an aggregate, must exist\n      either in srcAttrs or in srcDims.\n    - 0 or more aggregate calls. Each aggregate call has an\n      AGGREGATE_FUNC, an inputAttr and a resultName. The default\n      resultName is inputAttr followed by '_' and then AGGREGATE_FUNC.\n      The resultNames must already exist in outputAttrs.\n\nNotes\n-----\n\n    - The synthetic dimension cannot co-exist with aggregates. That is,\n      if there exists at least one aggregate, the synthetic dimension\n      must not exist.\n    - When multiple values are 'redimensioned' into the same cell in\n      outputArray, the collision handling depends on the schema: (a) If\n      there exists a synthetic dimension, all the values are retained\n      in a vector along the synthetic dimension. (b) Otherwise, for an\n      aggregate attribute, the aggregate result of the values is\n      stored. (c) Otherwise, an arbitrary value is picked and the rest\n      are discarded.", 
    "name": "redimension_store", 
    "signature": [
      "array", 
      "array", 
      "args"
  ...
  {
    "doc": "sum( srcArray [, attr {, groupbyDim}*] )\n\nProduces a result array containing the sum value among the values of an attribute. If a list of groupbyDims is provided, one sum value is calculated for every distinct group.\n\nParameters\n----------\n\n    - srcArray: a source array with srcAttrs and srcDims.\n    - 0 or 1 attributes from srcAttrs. If no attribute is provided, the\n      first attribute is used.\n    - an optional list of groupbyDims.\n\nNotes\n-----\n\n    - same as aggregate(srcArray, sum(attr), groupbyDims).", 
    "name": "sum", 
    "signature": [
      "array", 
      "args"
  ...
  {
    "doc": "stdev( srcArray [, attr {, groupbyDim}*] )\n\nProduces a result array containing the standard deviation among the values of an attribute. If a list of groupbyDims is provided, one standard deviation is calculated for every distinct group.\n\nParameters\n----------\n\n    - srcArray: a source array with srcAttrs and srcDims.\n    - 0 or 1 attributes from srcAttrs. If no attribute is provided, the\n      first attribute is used.\n    - an optional list of groupbyDims.\n\nNotes\n-----\n\n    - same as aggregate(srcArray, stdev(attr), groupbyDims).", 
    "name": "stdev", 
    "signature": [
      "array", 
      "args"
  ...
  {
    "doc": "var( srcArray [, attr {, groupbyDim}*] )\n\nProduces a result array containing the variance among the values of an attribute. If a list of groupbyDims is provided, one variance is calculated for every distinct group.\n\nParameters\n----------\n\n    - srcArray: a source array with srcAttrs and srcDims.\n    - 0 or 1 attributes from srcAttrs. If no attribute is provided, the\n      first attribute is used.\n    - an optional list of groupbyDims.\n\nNotes\n-----\n\n    - same as aggregate(srcArray, var(attr), groupbyDims).", 
    "name": "var", 
    "signature": [
      "array", 
      "args"
